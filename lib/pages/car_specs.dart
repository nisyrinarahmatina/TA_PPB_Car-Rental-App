...
class _DetailPageState extends State<DetailPage> {
  late Future<List<CarSpecs>> carSpecs;

  @override
  void initState() {
    super.initState();
    carSpecs = fetchCarDetails(widget.itemId);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: FutureBuilder(
          builder: (context, AsyncSnapshot<List<CarSpecs>> snapshot) {
            if (snapshot.hasData) {
              return Center(
...
Future<List<CarSpecs>> fetchCarDetails(int itemId) async {
  ...
  if (response.statusCode == 200) {
    ...
    return carsJson.map((car) => CarSpecs.fromJson(car)).toList();
  } else {
